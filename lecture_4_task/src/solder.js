...
var Solder = function (type) {
    ...
    this.lastHPchange = 0; //Дополнительное поле, необходимое для появления всплывающей дельты здоровья
    ...
    this.onHealAnimation = function () { //Дополнительная функция для получения лечения
    };
    this.attackTime = 0;
};
...
//Дополнительная функция для получения лечения
Solder.prototype.takeHeal = function (healNumber) {
    this.hp += healNumber;
    this.lastHPchange = healNumber;
    console.log(this.type + ' healed, hp - ' + this.hp);
    this.onHealAnimation();
};
